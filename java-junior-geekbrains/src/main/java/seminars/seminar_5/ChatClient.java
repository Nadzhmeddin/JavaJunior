        ...
        try (Socket server = new Socket("localhost", 8888)) {
            ...
            try (PrintWriter out = new PrintWriter(server.getOutputStream(), true)) {
                ...
                if(!checkLoginResponse(loginResponseString)) {
                    ...
                    //  (логин занят, ошибка аутентификации и тд)
...
//                new Thread(serverListener).start();
                ...
                while (true) {
                    ...
                    if(type.equals("1")) {
                        ...
                        request.setRecipient("unknown"); // TODO: Указываем логин получателя, вставляем считанный с консоли
                        ...
                        // TODO: Получить список всех клиентов
...
//                        });
    ...
    private static String createLoginRequest(String login) {
        ...
        try {
            return objectMapper.writer().writeValueAsString(loginRequest);
    ...
    private static void sleep() {
        try {
            Thread.sleep(Duration.ofMinutes(5).toMinutes());
